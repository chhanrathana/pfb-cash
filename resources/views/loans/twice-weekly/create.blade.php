@extends('layouts.app')
@section('title','បញ្ចូល-កម្ចីពីរសប្តាហ៍')

@section('content')

    @include('includes.alert-info')

    @include('includes.search-client', ['url' => route('loan.twice-weekly.create')])

    <form action="{{ route('loan.twice-weekly.store') }}" method="POST">
        @csrf
        @include('includes.create-client')

        <div class="card">
            <div class="card-header bg-custom"> <strong>ព័ត៌មានប្រាក់កម្ចីពីរសប្តាហ៍</strong></div>
            <div class="card-body">
                <div class="row">
